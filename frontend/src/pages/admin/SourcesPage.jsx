...
export default function SourcesPage() {
  ...
  const [tick,    setTick]    = useState(0);   // incrémenter pour forcer un re-fetch
  const [modal,   setModal]   = useState(false);
  const [form,    setForm]    = useState(EMPTY);
  const [saving,  setSaving]  = useState(false);
  const [error,   setError]   = useState(null);

  // ✅ IIFE async inline — aucune fonction externe avec setState appelée depuis useEffect
  useEffect(() => {
    let alive = true;
    (async () => {
      setLoading(true);
        ...
        <div className="card table-card">
          <table className="admin-table">
            <thead><tr>
              ...
              <th>Latence</th><th>Req./j</th><th>Limite</th><th>Synchro</th><th>Actions</th>
            </tr></thead>
            <tbody>
              {sources.map((s) => (
                <tr key={s.id}>
                  ...
                  <td><span className={`badge badge--${COLORS[s.statut]||'gray'}`}>{s.statut?.replace('_',' ')}</span></td>
              ...
              <div className="form-row">
                ...
                <div className="form-group"><label>Limite req./jour</label>
                  <input type="number" value={form.limiteRequetes}
                    onChange={(e) => setForm({...form,limiteRequetes:e.target.value})} placeholder="illimité" /></div>
              </div>
...
function fmt(iso){if(!iso)return'—';return new Date(iso).toLocaleString('fr-FR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'});}