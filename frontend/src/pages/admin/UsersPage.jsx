...
export default function UsersPage() {
  ...
  // ✅ fetchUsers ne contient AUCUN setState — il retourne une Promise.
  //    setLoading/setUsers sont appelés dans .then/.finally (asynchrone).
  //    useEffect ne fait qu'appeler fetchUsers() sans aucun setState direct.
  ...
  function openCreate() { setForm(EMPTY); setEditUser(null); setModal('create'); }
  function openEdit(u)   { setForm({ nom: u.nom, email: u.email, motDePasse: '', role: u.role, statut: u.statut }); setEditUser(u); setModal('edit'); }
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (modal === 'create') {
        ...
        const { motDePasse: _pwd, ...rest } = form;
      ...
      {loading ? <p className="page-loading">Chargement…</p> : (
        <div className="card table-card">
          <table className="admin-table">
            <thead><tr><th>Nom</th><th>Email</th><th>Rôle</th><th>Statut</th><th>Projets</th><th>Actions</th></tr></thead>
            <tbody>
              {users.map((u) => (
                <tr key={u.id}>
                  <td><div className="user-cell"><span className="avatar avatar--sm">{initials(u.nom)}</span>{u.nom}</div></td>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {modal === 'create' && (
                <div className="form-group"><label>Mot de passe</label>
                  <input required type="password" value={form.motDePasse} onChange={(e) => setForm({ ...form, motDePasse: e.target.value })} /></div>
              )}
